...
# The venv Python is an absolute symlink to /opt/conda/bin/python, which exists
# only inside the container. Check the regular MACS3 entry point from the host;
# the command itself is then executed and validated inside the container.
[[ -x "$environment/bin/macs3" ]] || { echo "Missing MACS3 environment: $environment" >&2; exit 1; }
